  ...
  return (
    <header
      className={`fixed top-0 inset-x-0 z-50 transition-all duration-500 ease-smooth ${
        scrolled
          ? 'bg-charcoal/70 backdrop-blur-xl border-b border-gold/15 shadow-glass'
    ...
    >
      <nav className="mx-auto max-w-7xl px-6 lg:px-10 flex items-center justify-between h-20">
        ...
        <div className="md:hidden flex items-center gap-4">
          ...
          <button
            ...
            className="relative h-10 w-10 flex flex-col items-center justify-center gap-1.5 text-ivory"
          >
            <span
      ...
      >
        <div className="bg-charcoal/95 backdrop-blur-xl border-t border-gold/10 px-6 py-6 flex flex-col gap-5">
          {NAV_LINKS.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              onClick={() => setMenuOpen(false)}
              className="font-body text-base text-ivory/85 hover:text-gold transition-colors duration-300"
            >
              {link.label}
            </Link>
          ))}
...
function CartLink({ itemCount, compact = false }) {
  return (
    <Link
      ...
      className={`relative flex items-center justify-center text-ivory/80 hover:text-gold transition-colors duration-300 ${
    ...
    >
      ...
      {itemCount > 0 && (
        <span className="absolute -top-1 -right-1 h-[1.125rem] min-w-[1.125rem] px-1 rounded-full bg-terracotta text-[10px] leading-[1.125rem] text-center text-ivory font-semibold">
          {itemCount}
        </span>
      )}
    </Link>
